...
def update_bn2(loader, loader2, model, device=None):
    ...
    for input, input2 in zip(loader, loader2):
        if isinstance(input, (list, tuple)):
            ...
            input2 = input2[0]
        if device is not None:
            x_l = input.cuda(non_blocking=True)
            x_ul = input2.cuda(non_blocking=True)

        #model(input)
        #model(x_l=x_l.detach(), x_ul=x_ul.detach())
        enc = model.module.module.encoder(x_l.detach())
        enc = model.module.module.classifier(enc)
...
def update_bn(loader, model, device=None):
    ...
    with tqdm.tqdm(loader, ascii=True) as tq:
        for input in tq:
            ...
            #model(input)
            model.module(input.detach())
            model.module(fliplr(input.detach()))
